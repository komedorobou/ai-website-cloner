...
2. GSAP ScrollTrigger / Motion (旧Motion (v12)) / Lenis によるApple級スクロールアニメーション
...
Phase 4: Animation（★NEW フェーズ）
  [7] AnimationImplementer — GSAP/Motion(旧Motion (v12))でアニメーション実装
...
**SKILL.md Phase 2 改造ノート:**
既存SKILL.mdのPhase 2は「Do it yourself (not delegated to an agent)」と記載。
MotionArchitectを並列dispatchするため、この制約を以下のように緩和する：
- DesignTokenExtractor（Agent 2）: 既存通りオーケストレーターが直接実行（globals.css更新のため）
- MotionArchitect（Agent 3）: worktreeにdispatch可能（読み取り専用でdocs/にのみ出力、共有ファイルを変更しない）
- AssetCurator（Agent 4）: worktreeにdispatch可能（public/にのみ出力）
Phase 2の「Foundation First」原則は維持。globals.css と TypeScript types はオーケストレーターが完了させてからPhase 3に進む。
...
**重要な互換性ノート:**
- `framer-motion` ではなく `motion` v12を使用（React 19対応版。framer-motion 11はReact 19と非互換）
- importは `motion/react` から行う（`framer-motion` からではない）
- GSAP ScrollTrigger はGSAPに含まれる（プラグイン登録が必要）
- `@gsap/react` は `useGSAP()` フックを提供
- Three.js は初期スコープ外。必要になったら追加。

**"use client" ルール:**
GSAP / Motion / Lenis はすべてクライアント専用ライブラリ。Next.js 16 App Router (React Server Components) 環境では、これらを使用するすべてのコンポーネント・フックに `"use client"` ディレクティブが必須。
...
- GSAP ScrollTrigger / Motion (v12) / Lenis の初期化
...
- **pin:** true/false
- **実装方針:** GSAP ScrollTrigger / Motion (v12) useInView / CSS animation
```
...
- `ScrollReveal.tsx` — viewport 進入時のフェード/スライドアニメーション（Motion (v12)）
...
3. GSAP ScrollTrigger / Motion (v12) のどちらで実装するかを判断
...
**コンポーネント修正戦略:**
Agent 5 (LayoutEngineer) → Agent 6 (TypographyRefiner) → Agent 7 (AnimationImplementer) は同じコンポーネントファイルを順次編集する。
各エージェントは前のエージェントの成果物を壊さないよう、**ターゲット編集**（既存コードにアニメーションラッパーを追加、import追加、props追加）を行う。フルリライトは禁止。
...
4. 重大な差異は自動修正
**差異判定基準:**
- レイアウト崩れ（要素の重なり、はみ出し、消失）→ 重大（自動修正）
- 余白・フォントサイズの明らかな不一致 → 重大（自動修正）
- 微細な差異（1-2pxのずれ等）→ 軽微（レポートのみ、PixelPolisherに委任）
